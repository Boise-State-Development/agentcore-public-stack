...
@Component({
  ...
  template: `
    ...
    <div class="rounded-md bg-linear-to-br from-violet-500/30 to-indigo-500/30 p-px">
      <div class="rounded-[calc(0.375rem-1px)] bg-violet-50/50 dark:bg-violet-950/30 px-2 py-1.5">
        <!-- Header with toggle -->
        <button
          type="button"
          class="flex w-full items-center gap-1.5 border-none bg-transparent p-0 font-inherit cursor-pointer transition-opacity duration-150 hover:opacity-80 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-violet-500 focus-visible:rounded-xs"
        ...
        >
          ...
          <ng-icon
            ...
            class="size-3 shrink-0 text-gray-500 dark:text-gray-400 transition-transform duration-200"
          ...
          <span class="flex flex-1 items-center gap-2 min-w-0">
            <span class="shrink-0 text-sm/5 font-medium text-gray-700 dark:text-gray-300">Thinking</span>
            @if (!isExpanded() && hasReasoningText()) {
              <span class="truncate text-xs text-gray-500 dark:text-gray-400 italic min-w-0">{{ previewText() }}</span>
            }
          </span>
        ...
        >
          <div class="overflow-hidden">
            <div class="pt-2">
              @if (hasReasoningText()) {
                <div class="max-h-80 overflow-y-auto whitespace-pre-wrap break-words rounded-xs border border-violet-200/50 dark:border-violet-700/30 bg-violet-100/30 dark:bg-violet-900/20 p-2 font-mono text-xs/relaxed text-gray-700 dark:text-gray-300">
                  {{ reasoningText() }}
                </div>
              }
