...
4. **Fine-tuning turns on with this deploy.** It is default-ON with a kill switch. To keep it dark, set `CDK_FINE_TUNING_ENABLED=false` explicitly. Also confirm `CDK_FINE_TUNING_DEFAULT_QUOTA_HOURS` — the existing repo value of `10` will now actually auto-grant 10 monthly GPU-hours to every signed-in user; `0` restores whitelist-only. Disabling leaves storage intact, so no dataset or trained model is orphaned.
