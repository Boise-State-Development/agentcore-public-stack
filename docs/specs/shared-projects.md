...
- **1.1 Infra:** `ProjectsConstruct` (table + OwnerIndex + MemberIndex, refs threaded through `PlatformComputeRefs`, grants to app-api + inference-api, `PROJECTS_ENABLED` env), `ProjectSessionIndex` on sessions-metadata (its own PR), `config.ts` ternary + `platform.yml` var, jest tests, table-count bump.
