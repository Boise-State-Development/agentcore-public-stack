...
> ⚠️ **Queue-health notice added by research/2026-08-14.** The `bedrock-agentcore` bump entries below ([2026-07-24], [2026-07-17], [2026-07-10]) and the [2026-07-24] nightly item assert premises that are **now factually false** — the bump shipped in #857 (1.9.1 → **1.21.0**, zero lag; #482 and #571 closed upstream), and Nightly has been green 12 consecutive days. See the [2026-08-14] queue-hygiene item below; resolve those four before ranking.
...
- **Effort × Impact**: L × M–H
- **Subtracts**: yes — retires 4 superseded entries, 3 of which chain through the same subject and would otherwise be re-ranked as live work
- **Status**: open — **do this first; ~20 minutes, and it de-risks the ranking run.** Resolve as *superseded by events*: (a) the three `bedrock-agentcore` bump entries — **shipped in #857**, 1.9.1 → **1.21.0**, zero lag, #482 + #571 closed upstream; carry forward only the real residue, **#564 is still open** and wants a local guard; (b) the [2026-07-24] nightly `DELETE_FAILED` item — **resolved**, 12 consecutive green nights; (c) the [2026-07-10] Strands entry whose own status line says the bump shipped — split the un-adopted capabilities (`continue_on_error`, `Limits` on the headless lane) into their own entries and close the stub; (d) **down-rank, don't close**, the [2026-07-17] curl-pin item — the `deb13u*` wildcard is holding, so it's correctness debt rather than active breakage. This skill does not edit `## Resolved` — that move is review-prep's job, and this entry is the instruction to make it.
