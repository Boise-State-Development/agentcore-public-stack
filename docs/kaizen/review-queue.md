...
### [2026-09-05] Check PROD for the `gpt-5.4` cache-rate gap — costs understated AND savings overstated
- **Source**: live measurement in dev while adding the GPT-5.6 family, 2026-09-05. Not a research scan.
- **Surface**: data, not code — the `openai.gpt-5.4` managed-model row in **prod**. The code fix already shipped (#963).
- **Effort × Impact**: L × M–H — one admin edit; the impact is that every cost figure quoted for that model in prod is currently wrong in two directions at once.
- **Subtracts**: no — it corrects a record.
- **Status**: open. Dev is fixed and verified; **prod has not been checked.** `openai.gpt-5.4` caches implicitly, and with `cacheReadPricePerMillionTokens` unset the platform (a) prices cached tokens at **$0.00** and (b) credits them as a **100%** cache saving instead of 90% — the same missing field driving both. Measured in dev: a warm turn read 3,642 tokens from cache and contributed nothing; after setting the rate the same turn shape went $0.000264 → $0.0012655 (**4.8×**). Prod's row predates all of this and carries real traffic, so this is the follow-up with actual money attached. Fix: `supportsCaching: true`, `cacheReadPricePerMillionTokens` = 0.1 × the row's input rate, `cacheWritePricePerMillionTokens: 0` (the Price List has a cache-read SKU and **no** cache-write SKU for this model). ⚠️ Prod is hook-enforced read-only for this agent, so it needs a human. ⚠️ Check the other Mantle rows at the same time — the caching controls were hidden for **all** Mantle models until #963, so any Mantle model that caches has the same shape.

### [2026-09-05] Decide whether to request an SCP exception for `global.*` Bedrock inference profiles
- **Source**: live failure in dev while adding GPT-5.6 Luna, 2026-09-05.
- **Surface**: AWS Organization policy (`arn:aws:organizations::977099011063:policy/o-09d6ih8vwl/service_control_policy/p-r61tynkc`) — outside this account. No code.
- **Effort × Impact**: L to ask × M — a standing ~9% discount on the GPT-5.6 family if granted.
- **Subtracts**: no.
- **Status**: open — needs someone with Organization access, not an engineering change. `global.openai.gpt-5.6-luna` fails with an **explicit deny** on the region-less ARN `arn:aws:bedrock:::foundation-model/openai.gpt-5.6-luna`; the *same row* edited to `us.` succeeds, which isolates the profile prefix as the cause. This contradicts the cost model in `docs/specs/gpt-5-6-prompt-caching.md`, which prefers `bedrock-runtime` partly because Global CRIS is cheaper for this family ($4.00 vs $4.40 input on Sol) — that saving is **unavailable while the SCP stands**. Until resolved, every GPT-5.6 row must use `us.*` and no projection should assume the Global discount. ⚠️ Note this is the **opposite** of the Claude rule of thumb, where `us.*` costs ~10% *more* than `global.*` — so the instinct to reach for `global.` is right on cost and blocked on policy.
