...
- [ ] 1. Pin GitHub Actions to SHA digests and standardize checkout version
  - [ ] 1.1 Pin all third-party action references in workflow YAML files to SHA digests with version comments
  ...
  - [ ] 1.2 Pin third-party actions inside the composite action to SHA digests
  ...
  - [ ]* 1.3 Write property test for SHA pinning (Property 1)
  ...
  - [ ]* 1.4 Write property test for consistent checkout SHA (Property 10)
...
- [ ] 2. Pin runner versions across all workflows
  - [ ] 2.1 Replace all `ubuntu-latest` with `ubuntu-24.04` in workflow files
  ...
  - [ ]* 2.2 Write property test for runner version pinning (Property 7)
...
- [ ] 3. Checkpoint — Verify workflow YAML changes
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 4. Pin Python dependencies to exact versions and fix mypy version
  - [ ] 4.1 Replace all `>=`, `~=`, and unpinned versions in `pyproject.toml` with `==` exact pins
  ...
  - [ ] 4.2 Fix mypy `python_version` to match `requires-python`
    - In `backend/pyproject.toml`, change `[tool.mypy] python_version = "3.9"` to `python_version = "3.10"` to match `requires-python = ">=3.10"`
    - _Requirements: 12.1_

  - [ ]* 4.3 Write property test for Python dependency pinning (Property 2)
...
- [ ] 5. Pin frontend and infrastructure npm dependencies to exact versions
  - [ ] 5.1 Pin all frontend dependencies in `frontend/ai.client/package.json` to exact versions from lockfile
  ...
  - [ ] 5.2 Pin infrastructure dependencies in `infrastructure/package.json` to exact versions
  ...
  - [ ]* 5.3 Write property test for npm dependency pinning (Property 3)
...
- [ ] 6. Harden install scripts (global tool pinning + npm ci enforcement)
  - [ ] 6.1 Pin CDK CLI version in install scripts
  ...
  - [ ] 6.2 Replace `npm install` with `npm ci` and add lockfile checks in install scripts
  ...
  - [ ]* 6.3 Write property test for global npm install pinning (Property 4)
  ...
  - [ ]* 6.4 Write property test for npm ci enforcement (Property 5)
...
- [ ] 7. Checkpoint — Verify dependency and script changes
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 8. Pin Docker apt-get/dnf package versions
  - [ ] 8.1 Pin apt-get package versions in `Dockerfile.app-api` and `Dockerfile.inference-api`
  ...
  - [ ] 8.2 Pin dnf package versions in `Dockerfile.rag-ingestion` where practical
  ...
  - [ ]* 8.3 Write property test for Dockerfile package pinning (Property 9)
...
- [ ] 9. Add container image scanning as nightly track
  - [ ] 9.1 Add `scan-images` track resolution to `nightly.yml`
  ...
  - [ ] 9.2 Add `scan-images` job to `nightly.yml`
...
- [ ] 10. Scope secrets to AWS-using jobs only
  - [ ] 10.1 Audit and move AWS credentials from workflow-level to job-level env blocks
  ...
  - [ ]* 10.2 Write property test for secret scoping (Property 14)
...
- [ ] 11. Checkpoint — Verify Docker, scanning, and secret scoping changes
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 12. Create documentation files
  - [ ] 12.1 Create `CONTRIBUTING.md` at repository root
  ...
  - [ ] 12.2 Create `.github/ARTIFACT_RETENTION.md`
  ...
  - [ ]* 12.3 Write property test for artifact retention consistency (Property 11)
  ...
  - [ ]* 12.4 Write property test for cancel-in-progress on deploy workflows (Property 12)
  ...
  - [ ]* 12.5 Write unit tests for documentation and mypy version
...
- [ ] 13. Validate Dependabot configuration (no changes needed)
  - [ ]* 13.1 Write property test for Dependabot config (Property 8)
