...
| CDK_DICTATION_ENABLED | Variable | No | `true` | App API | Composer dictation (speech-to-text via Amazon Transcribe Streaming). Sets the container's `DICTATION_ENABLED`; `false` makes `/dictation/*` 404 and the SPA hides the Dictate button. The Transcribe IAM grant is provisioned either way. |
| CDK_DICTATION_LANGUAGES | Variable | No | `en-US` | App API | Comma-separated Transcribe language codes for dictation. One code pins the language; two or more (e.g. `en-US,es-US`) enable automatic language identification, first code preferred. One dialect per language. |
